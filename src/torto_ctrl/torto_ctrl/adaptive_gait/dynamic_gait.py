...
class trotGait():
    ...
    def calculateStance(self , phi_st , Vx, Vy, angle):#phi_st between [0,1), angle in degrees
    ...
    def calculateBezier_swing(self , phi_sw , Vx, Vy, Vz, angle):#phi between [0,1), angle in degrees
...
##            print('foot UP', self.s , phi)
    ...
    def stepTrajectory(self , phi , Vx, Vy, Vz, angle , Wrot, centerToFoot): #phi belong [0,1), angles in degrees
        if (phi >= 1):
            phi = phi - 1.
        ...
        if phi <= stepOffset: #stance phase
            ...
            stepX_long , stepY_long , stepZ_long = self.calculateStance(phiStance , Vx, Vy, angle)#longitudinal step
            stepX_rot , stepY_rot , stepZ_rot = self.calculateStance(phiStance , Wrot, Wrot, circleTrayectory)#rotational step
#            print(phi,phiStance, stepX_long)
        else: #swing phase
            phiSwing = (phi-stepOffset)/(1-stepOffset)
            stepX_long , stepY_long , stepZ_long = self.calculateBezier_swing(phiSwing , Vx, Vy, Vz, angle)#longitudinal step
            stepX_rot , stepY_rot , stepZ_rot = self.calculateBezier_swing(phiSwing , Wrot, Wrot, Wrot, circleTrayectory)#rotational step
    ...
    def loop(self , Vx, Vy, Vz, angle , Wrot, T , offset , bodytoFeet_ ):
...
#        print(self.phi)
        ...
        step_coord = self.stepTrajectory(self.phi + offset[0] , Vx, Vy, Vz, angle , Wrot, np.squeeze(np.asarray(bodytoFeet_[0,:]))) #FR
        ...
        step_coord = self.stepTrajectory(self.phi + offset[1] , Vx, Vy, Vz, angle , Wrot, np.squeeze(np.asarray(bodytoFeet_[1,:])))#FL
        ...
        step_coord = self.stepTrajectory(self.phi + offset[2] , Vx, Vy, Vz, angle , Wrot, np.squeeze(np.asarray(bodytoFeet_[2,:])))#BR
        ...
        step_coord = self.stepTrajectory(self.phi + offset[3] , Vx, Vy, Vz, angle , Wrot, np.squeeze(np.asarray(bodytoFeet_[3,:])))#BL
